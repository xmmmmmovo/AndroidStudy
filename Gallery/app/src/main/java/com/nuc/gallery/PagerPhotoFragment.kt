...
class PagerPhotoFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        var photoList = arguments?.getParcelableArrayList<PhotoItem>("PHOTO_LIST")
    ...
    private suspend fun savePhoto() {
        ...
        withContext(Dispatchers.IO) {
            val holder = ((viewPager2[0] as RecyclerView)
            ...
            val saveUri = requireContext().contentResolver.insert(
                ...
                MainScope().launch {
                    Toast.makeText(
                        requireContext(), "存储失败", Toast.LENGTH_LONG
                    ).show()
                }
                return@withContext
            }
            requireContext().contentResolver
                ...
                .use {
                    if (bitmap.compress(
                        ...
                        MainScope().launch {
                            Toast.makeText(
                                requireContext(), "存储成功！", Toast.LENGTH_LONG
                            ).show()
                        }
                    } else {
                        MainScope().launch {
